...
async def enjdleech(event, args: str, client, direct=False):
    """
    Adds a link to encoding queue using JDownloader:
        Requires a reply to link or the link as argument
        can also add consecutive items to queue by replying
        to the first link and a number of how many links to add to queue
    Accepts the following flags:
    ...
    if not jd_available:
        return await event.reply(
            "`JDownloader is not available or not configured properly.`\n"
            "Please check your JD_EMAIL and JD_PASSWORD in config."
        )
    ...
    invalid_msg = "`Invalid direct link`"
    no_uri_msg = "`uhm you need to reply to or send command alongside a uri/direct link`"
    no_dl_spt_msg = "`File to download is…\neither not a video\nor is a batch which is currently not supported for JDownloader.`"
            ...
            if args:
                ...
                async with queue_lock:
                    for _none, _id in zip(
                        ...
                        if event2.empty:
                            await event.reply(
                                f"Resend uri links and try replying the first with /jl or /jleech again"
                            )
                            return await rm_pause(dl_pause, 5)
                        uri = event2.text
                        if not uri:
                        ...
                        # Get file info using JDownloader
                        file = await get_jd_leech_name(uri)
                        if file.error:
                            await event2.reply(f"`{file.error}`", quote=True)
                            await asyncio.sleep(10)
                            continue
                        ...
                        queue.update(
                            {
                                (chat_id, event2.id): [
                                    ...
                                    (
                                        cust_v or get_v(),
                                        cust_fil or get_f(),
                                        ("jd", mode),  # Using 'jd' as downloader
                                        force_name,
                                        (anilist, uri),
                                    ),
                                ]
                        ...
                        msg = await event2.reply(
                            f"**Link added to queue ⏰ (JDownloader), POS:** `{len(queue)-1}`\n`Please Wait , Encode will start soon`",
                            quote=True,
                        )
                        await asyncio.sleep(5)
                    if len(queue) > 1:
